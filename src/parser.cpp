...
void vecParser(std::istream &inStream, std::vector<std::vector<std::string>> &commVector, const bool &throwComment)
{
    std::string inComm;
    ...
    while (1)
    {
        if (!(inStream >> inComm) || !(addVec(inStream, commVector, count, inComm, throwComment)))
            break;
    }
}

bool addVec(std::istream &inStream, std::vector<std::vector<std::string>> &commVector, unsigned int &count, std::string func, const bool &throwComment)
{
    if (func == "exit")
        return false;
    else if (func[0] == '#')
    {
        ...
        if (!throwComment)
        {
            // this is mainly for formatter purposes, it will make the comment on the next line
            commVector.push_back({"", "# " + trim(throwAway)});
        }
    }
    else if (func.back() == ':')
    {
    ...
    // }
        ...
        exitError("Invalid instruction \"" + func + "\"", ((count - 0x10000000) / 4 + 1), 5);
    else
    {
        ...
        if (func == "jalr")
        {
            ...
            if (!(inStream >> arg1))
                exitError("Not enough arguments for final instruction.", ((count - 0x10000000) / 4 + 1), 5);
            if (!(inStream >> arg2))
            {
